...
async function creditBadge(
  ...
  return db.transaction(async (tx) => {
    const inserted = await tx
      .insert(achievements)
      .values({ userId, badgeId: def.id, progress: def.target })
      .onConflictDoNothing()
...
/**
 * 评估 + 解锁 + 返回全量徽章状态。
 * 顺序：先 7 枚非 master → 统计已解锁数 → 再评估 master（艾比大师）。
 */
export async function syncAchievements(
  ...
  for (const def of nonMaster) {
    ...
    if (await creditBadge(userId, def)) {
  ...
  if (!unlockedMap.has("master") && nonMasterUnlocked >= masterDef.target) {
    if (await creditBadge(userId, masterDef)) {
