...
export function buildXhsPrompt(ctx: PsychologyContext): string {
  return `你是「艾比世界」（Aibi World，AI 虚拟宠物陪伴产品）的运营数字人，负责在小红书账号自动营业。
...
export function buildXPrompt(ctx: PsychologyContext): string {
  return `You are the digital human of "Aibi World" (an AI virtual pet companion product) operating the X/Twitter account automatically.
...
#艾比世界 #AI陪伴 #治愈系 #情绪价值`;
  ...
  return `Aibi World keeps getting better: ${head}
