...
const RSS_SOURCES = [
  { name: "The Guardian · Animals", url: "https://www.theguardian.com/world/animals/rss", weight: 900 },
  { name: "ScienceDaily · Animals", url: "https://www.sciencedaily.com/rss/animals_insects.xml", weight: 600 },
  { name: "PETA", url: "https://www.peta.org/feed/", weight: 550 },
  { name: "V2EX", url: "https://www.v2ex.com/index.xml", weight: 500 },
  { name: "少数派", url: "https://sspai.com/feed", weight: 500 },
];
...
function parseRss(xml: string, source: string, weight: number): RawItem[] {
  const items: RawItem[] = [];
...
export async function fetchAndStoreNews(): Promise<{ inserted: number; total: number; fallback: boolean }> {
  ...
  for (const s of RSS_SOURCES) {
    try {
      ...
      collected.push(...parseRss(xml, s.name, s.weight));
    } catch {
      // 源不可达：跳过
    }
  }
  ...
  const rows: { source: string; title: string; desc: string; cover: string | null; hot: number; timestamp: number; url: string }[] = [];
  ...
  if (!rows.length) {
    ...
    for (const s of SEED_NEWS) {
      rows.push({ source: s.source, title: s.title, desc: s.desc ?? "", cover: s.cover, hot: computeHotScore(1200, s.timestamp, now), timestamp: s.timestamp, url: s.url ?? "" });
    }
  }

  // 幂等写入（source+title 唯一，冲突跳过）
  let inserted = 0;
  if (rows.length) {
    const values = rows.flatMap((r) => [r.source, r.title, r.desc, r.cover, r.hot, r.timestamp, r.url]);
    const placeholders = rows.map((_, i) => `($${i * 7 + 1},$${i * 7 + 2},$${i * 7 + 3},$${i * 7 + 4},$${i * 7 + 5},$${i * 7 + 6},$${i * 7 + 7})`).join(",");
    const res = await pool.query(
      `INSERT INTO hotnews (source, title, "desc", cover, hot, timestamp, url)
       VALUES ${placeholders}
