...
const qrText =
  ...
  (fs.existsSync(path.join(__dirname, "_realpay.qr.txt"))
    ...
    : "");
const out = process.argv[3] || path.join(__dirname, "_realpay.qr.svg");
...
<body style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;margin:0;font-family:system-ui;background:#f5f5f5">
  ...
  <p style="margin-top:20px;color:#888;font-size:12px">支付完成后运行：node scripts/pay-acceptance-check.cjs realpay_1651453@test.aiabw</p>
</body></html>`;
